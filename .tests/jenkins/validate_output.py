...
def validate_result(json_file, directory):
    with open(json_file) as input_data:
        ...
        log.info("Validating")
        for analys in validation_data:
            log.info(f"Step: {analys}")
            for rule in validation_data[analys]:
                log.info(f"- rule: {rule}")
                for f_path, md5 in validation_data[analys][rule]["files"].items():
                    if f_path.endswith("gz"):
                        result = process_gz_file(f_path, directory)
...
def process_gz_file(f_path, directory):
    ...
    command = ["zcat", os.path.join(directory, f_path), "|", "md5sum"]
